export function fade_element_in(
  element,
  on_complete,
  interval = 10,
  increment = 0.005,
...
export function fade_multiple_elements_in(
  elements,
  on_complete,
  interval = 10,
  increment = 0.005,
...
export function fade_element_out(
  element,
  on_complete,
  interval = 10,
  increment = 0.005,
...
export function fade_multiple_elements_out(
  elements,
  on_complete,
  interval = 10,
  increment = 0.005,
  ...
  const id = setInterval(() => {
    ...
    if (opacity <= min_opacity) {
      elements.forEach(
        (element) => (element.style.opacity = min_opacity.toString()),
      );
      on_complete && on_complete();
      clearInterval(id);
    }
...
export function check_intersecting_rectangles(
  rect1_x1,
  rect1_y1,
  rect1_x2,
  rect1_y2,
