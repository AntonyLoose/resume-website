...
function swap_current_image() {
  ...
  const curr_background_index = landing_background_images.findIndex((img_src) =>
    id.includes(img_src.slice(2)),
  );
  const next_background_index =
    curr_background_index + 1 >= landing_background_images.length
      ? 0
      : curr_background_index + 1;
