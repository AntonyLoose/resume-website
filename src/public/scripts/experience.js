...
setInterval(() => {
  stars.forEach((star) => {
    apply_gravity(star, mouse_x, mouse_y, mouse_mass);
    star.tick();
  });
  if (elapsed % 500 == 0) {
...
auslan_ref.onclick = () => {
  const auslan_text_content =
    "ka<span class='spam-protection'>Seriously, the fact that I need to do this says a lot about the world</span>lin.stefan<!-- 123stopspam -->ov@mona<!-- 123stopspam -->sh.edu ";
...
ilearn_ref.onclick = () => {
  const ilearn_text_content =
    "devmym<!-- 123stopspam -->ai<span class='spam-protection'>No, go away</span>l1@gm<!-- 123stopspam -->ail.com";
...
intake_ref.onclick = () => {
  const intake_text_conent =
    "nabeeb2<!-- 123stopspam -->000@g<span class='spam-protection'>Nice try buddy</span>mai<!-- 123stopspam -->l.com";
...
autom8_ref.onclick = () => {
  const autom8_text_content =
    "grah<!-- 123stopspam -->am@35<span class='spam-protection'>Go away please.</span>007.<!-- 123stopspam -->com.au";
...
woolies_ref.onclick = () => {
  const woolies_text_content =
    "042<!-- 123stopspam -->6 16<span class='spam-protection'>No, I don't want you messaging my ex-manager saying this website is out of date, thanks and goodbye.</span>9 41<!-- 123stopspam -->7";
