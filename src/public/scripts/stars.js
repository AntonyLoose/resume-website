  ...
  #_apply_friction() {
    ...
    if (this.#_vx != 0) {
      this.#_update_vx(
        (this.#_vx -=
          this.#_vx > 0
            ? constant_speed_loss_due_to_friction
            : -constant_speed_loss_due_to_friction),
      );
    }
    if (this.#_vy != 0) {
...
export function generate_random_star(
  min_size,
  max_size,
  min_fade_in_time,
  max_fade_in_time,
  ...
  const fade_in_time =
    min_fade_in_time + (max_fade_in_time - min_fade_in_time) * Math.random();
  const fade_out_time =
    min_fade_out_time + (max_fade_out_time - min_fade_out_time) * Math.random();
  const life_time =
    min_life_time + (max_life_time - min_life_time) * Math.random();
  return new Star(
    x,
    y,
    size,
...
export function apply_gravity(
  ...
  fx =
    fx >= 0
      ? Math.min(fx, max_gravitational_force)
      : Math.max(fx, -max_gravitational_force);
  fy =
    fy >= 0
      ? Math.min(fy, max_gravitational_force)
      : Math.max(fy, -max_gravitational_force);
